...
description: "PrivateSettings.php: why it lives outside the repo, R2/S3 and OAuth key setup, and required variables with types — DB credentials, secret key, and Sentry DSN."
---

`PrivateSettings.php` is the first file included by `LocalSettings.php` — loaded via `require_once "$IP/config/PrivateSettings.php"` before any other configuration runs. It is **not part of the Git repository** and is deployed separately to `/srv/mediawiki/config/PrivateSettings.php` on each server. Its purpose is to hold every piece of information that would be dangerous to expose in version control: database passwords, secret keys, object-storage credentials, Sentry DSN, and OAuth private key paths. Without this file present on disk, the entire farm fails to start.
...
<Note>
  The OAuth **public** key (`OAuth.key.pub`) **is** committed to the repository at `/srv/mediawiki/config/OAuth.key.pub` and referenced by `$wgOAuth2PublicKey`. Only the private key (`OAuth.key`) is kept in `PrivateSettings.php` via `$wgOAuth2PrivateKey`. This is intentional — public keys are designed to be distributable.
</Note>
...
<Steps>
  ...
  <Step title="Set OAuth key paths">
    The OAuth private key path is configured here; the public key path is already set in `LocalSettings.php`:
    ```php
    // In PrivateSettings.php — private key only
    // $wgOAuth2PrivateKey is set in LocalSettings.php to:
    //   /srv/mediawiki/config/OAuth.key
    // Ensure that file exists and is readable by the web user.
    ```
    Generate an RSA key pair if one does not exist:
    ```bash
    ...
    ```
    Commit `OAuth.key.pub` to the repository. Keep `OAuth.key` on disk only.
  </Step>
