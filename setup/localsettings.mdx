...
description: "LocalSettings.php bootstrap deep-dive: load order, staging vs production detection, virtual-domain mappings, versioned paths, and $wgConf settings categories."
---
...
<Steps>
  ...
  <Step title="ManageWiki config files">
    `ManageWikiExtensions.php`, `ManageWikiNamespaces.php`, and `ManageWikiSettings.php` apply the per-wiki extension list and namespace/settings configuration stored in the database by ManageWiki.
  </Step>
  <Step title="Final includes">
    `Database.php`, `GlobalCache.php`, `GlobalSettings.php`, and `LocalWiki.php` are included last. `GlobalSettings.php` registers farm-wide hooks and conditionally loads optional extensions. See each file's documentation for details.
  </Step>
</Steps>
...
```
ManageWikiExtensions.php   → registers all available extensions in ManageWiki
